...
public class WadSamoyed {

    // ขนาดของพื้นที่วาดตัวละคร (เผื่อขนาดไว้ให้พอดี)
    private final int CANVAS_WIDTH = 300;
    private final int CANVAS_HEIGHT = 300;

    /**
     * Method หลักสำหรับควบคุมการวาดตัวละครทั้งหมด
     * จะรับ frameNumber เข้ามาเพื่อเลือกวาดเฟรมที่ถูกต้อง
     */
    public void draw(Graphics2D g, double x, double y, int frameNumber) {
        switch (frameNumber) {
            case 1:
                drawFrame1(g, x, y);
                break;
            ...
            //     drawFrame2(g, x, y);
            //     break;
            // ... เพิ่ม case สำหรับเฟรมอื่นๆ ที่นี่ ...
            default:
                // หากไม่มีเฟรมที่ระบุ ให้วาดเฟรม 1 เป็นค่าเริ่มต้น
                drawFrame1(g, x, y);
                break;
        }
    }
    ...
    /**
     * กระบวนการวาดและลงสีสำหรับ Frame 1
     */
    private void drawFrame1(Graphics2D g, double x, double y) {
        // 1. สร้าง Canvas (BufferedImage) สำหรับวาดเฟรมนี้โดยเฉพาะ
        BufferedImage frameCanvas = new BufferedImage(CANVAS_WIDTH, CANVAS_HEIGHT, BufferedImage.TYPE_INT_ARGB);
        
        // เราจะวาดทุกอย่างลงบน frameCanvas นี้ก่อน โดยใช้ตำแหน่ง (0,0) ของ canvas
        // แล้วค่อยนำ frameCanvas ทั้งหมดไปวาดลงบนหน้าจอหลักที่ตำแหน่ง (x,y)
        double drawX = 0;
        double drawY = 0;

        Graphics2D g2d_rough = frameCanvas.createGraphics();
        
        // 1. เตรียมพื้นหลังให้เป็นสีขาวทึบก่อน
        g2d_rough.setColor(Color.WHITE);
        g2d_rough.fillRect(0, 0, CANVAS_WIDTH, CANVAS_HEIGHT);
        
        // 2. วาดเส้นร่างแบบคมๆ (Anti-aliasing OFF) สำหรับการลงสี
        g2d_rough.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_OFF);
        drawFrame1_Outlines(g2d_rough, drawX, drawY);
        g2d_rough.dispose();

        // 3. ลงสีด้วย Flood Fill บนเส้นร่างที่คมชัด
        colorFrame1_Parts(frameCanvas, drawX, drawY);

        // 4. วาดเส้นจริงแบบสวยๆ (Anti-aliasing ON) ทับลงไป
        Graphics2D g2d_final = frameCanvas.createGraphics();
        g2d_final.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
        drawFrame1_Outlines(g2d_final, drawX, drawY);
        g2d_final.dispose();

        // 5. นำ Canvas ของเฟรมที่วาดเสร็จแล้ว ไปวาดลงบน Graphics หลักของเวที
        g.drawImage(frameCanvas, (int)x, (int)y, null);
    }
    
    // --- ต่อไปนี้คือ method สำหรับ Frame 2, 3, 4... ---
    // private void drawFrame2(Graphics2D g, double x, double y) { ... }
    // private void drawFrame2_Outlines(Graphics2D g2d, double x, double y) { ... }
    // private void colorFrame2_Parts(BufferedImage canvas, double x, double y) { ... }


    // ==================================================================
    // ==                 ส่วนประกอบของ FRAME 1                       ==
    // ==================================================================

    private void drawFrame1_Outlines(Graphics2D g2d, double x, double y) {
    ...
    private void colorFrame1_Parts(BufferedImage canvas, double x, double y) {
        // กำหนดเป้าหมายของเราให้เป็น "สีขาว" เสมอ
        int targetRGB = Color.WHITE.getRGB();

        // set color-shadow head
        floodFill(canvas, (int) x + 80, (int) y + 60, targetRGB, new Color(255, 195, 144).getRGB());
        // set color-shadow leg R
        floodFill(canvas, (int) x + 85, (int) y + 150, targetRGB, new Color(255, 195, 144).getRGB());
        // set color-shadow leg L
        floodFill(canvas, (int) x + 100, (int) y + 170, targetRGB, new Color(255, 195, 144).getRGB());
        // set color-shadow tail
        floodFill(canvas, (int) x + 50, (int) y + 130, targetRGB, new Color(255, 195, 144).getRGB());
        // set color-shadow hand R
        floodFill(canvas, (int) x + 50, (int) y + 100, targetRGB, new Color(255, 195, 144).getRGB());
        // set color-shadow shirt
        floodFill(canvas, (int) x + 60, (int) y + 85, targetRGB, new Color(255, 203, 159).getRGB());
        floodFill(canvas, (int) x + 70, (int) y + 120, targetRGB, new Color(255, 203, 159).getRGB());
        // set color-shadow hat
        floodFill(canvas, (int) x + 60, (int) y + 30, targetRGB, new Color(115, 71, 121).getRGB());
        floodFill(canvas, (int) x + 80, (int) y + 15, targetRGB, new Color(115, 71, 121).getRGB());
        floodFill(canvas, (int) x + 75, (int) y + 35, targetRGB, new Color(115, 71, 121).getRGB());
        floodFill(canvas, (int) x + 105, (int) y + 15, targetRGB, new Color(115, 71, 121).getRGB());
        // set color-shadow bow
        floodFill(canvas, (int) x + 62, (int) y + 74, targetRGB, new Color(115, 71, 121).getRGB());
        floodFill(canvas, (int) x + 85, (int) y + 90, targetRGB, new Color(115, 71, 121).getRGB());
        
        // set color-bow
        floodFill(canvas, (int) x + 105, (int) y + 105, targetRGB, new Color(115, 89, 190).getRGB());
        floodFill(canvas, (int) x + 120, (int) y + 110, targetRGB, new Color(115, 89, 190).getRGB());
        floodFill(canvas, (int) x + 120, (int) y + 98, targetRGB, new Color(115, 89, 190).getRGB());
        floodFill(canvas, (int) x + 140, (int) y + 100, targetRGB, new Color(115, 89, 190).getRGB());
        
        // set color-hat
        floodFill(canvas, (int) x + 120, (int) y + 15, targetRGB, new Color(115, 89, 190).getRGB());
        
        // set color-lin
        floodFill(canvas, (int) x + 135, (int) y + 75, targetRGB, new Color(255, 101, 101).getRGB());
        
        // set color-body
        floodFill(canvas, (int) x + 120, (int) y + 40, targetRGB, new Color(255, 245, 227).getRGB());
        floodFill(canvas, (int) x + 120, (int) y + 140, targetRGB, new Color(255, 245, 227).getRGB());
        floodFill(canvas, (int) x + 165, (int) y + 115, targetRGB, new Color(255, 245, 227).getRGB());
    }



    // ==================================================================
    // ==               HELPER METHODS (เครื่องมือช่วย)                ==
    // ==================================================================

    private void floodFill(BufferedImage canvas, int x, int y, int targetRGB, int replacementRGB) {
